...
def trace_exception(e):
    """Simple replacement for ascii_colors.trace_exception"""
    import traceback
...
def create_query_routes(rag, api_key: Optional[str] = None, top_k: int = 60):
    ...
    async def query_text(request: QueryRequest, graph_id: Optional[str] = None):
        """
        Handle a POST request at the /query endpoint to process user queries using RAG capabilities.

        Parameters:
            request (QueryRequest): The request object containing the query parameters.
            graph_id (Optional[str]): The graph ID to query against. If not provided, uses current active graph.
        Returns:
        ...
        try:
            # 获取RAG实例
            current_rag = await _get_rag_instance(graph_id, rag)

            param = request.to_query_params(False)
            response = await current_rag.aquery(request.query, param=param)
    ...
    async def query_text_stream(request: QueryRequest, graph_id: Optional[str] = None):
        """
