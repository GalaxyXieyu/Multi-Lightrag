...
import shutil
import traceback
# import pipmaster as pm  # Removed unused import

# Simple package manager replacement
...
# 导入多图谱支持
try:
    from lightrag.api.middleware import get_current_graph_id, get_current_graph_info
    from lightrag.storage import get_storage_manager
    MULTI_GRAPH_SUPPORT = True
except ImportError:
    MULTI_GRAPH_SUPPORT = False
    logger.warning("多图谱支持不可用")
...
class DocStatusResponse(BaseModel):
    ...
    # 新增：图谱信息
    graph_id: Optional[str] = Field(default="default", description="所属图谱ID")
    graph_name: Optional[str] = Field(default="Default Graph", description="所属图谱名称")
...
async def pipeline_index_file_with_graph(rag: LightRAG, file_path: Path, graph_id: str):
    """Index file with graph context"""
    if MULTI_GRAPH_SUPPORT:
        try:
            ...
            # 确保图谱存储结构存在
            await storage_manager.create_graph_storage(graph_id)

            # 获取图谱工作目录
            graph_working_dir = await storage_manager.get_graph_working_dir(graph_id)
            if not graph_working_dir:
                logger.error(f"无法获取图谱 {graph_id} 的工作目录")
                await pipeline_index_file(rag, file_path)
                return

            # 创建图谱专用的RAG实例
            from lightrag.api.routers.graph_routes import _graph_manager

            # 切换到目标图谱
            await _graph_manager.switch_graph(graph_id)
            current_rag = _graph_manager.current_rag

            if current_rag:
                # 使用图谱专用的RAG实例处理文件
                logger.info(f"使用图谱 {graph_id} 的RAG实例处理文件 {file_path}")
                await pipeline_index_file(current_rag, file_path)
                logger.info(f"文件 {file_path} 已成功处理到图谱 {graph_id}")
            else:
                logger.warning(f"无法获取图谱 {graph_id} 的RAG实例，使用默认处理")
                await pipeline_index_file(rag, file_path)

        except Exception as e:
            logger.error(f"处理图谱文件失败: {e}")
            import traceback
            logger.error(traceback.format_exc())
            # 回退到普通处理
            await pipeline_index_file(rag, file_path)
...
def create_document_routes(
    ...
    async def upload_to_input_dir(
        ...
        Args:
            ...
            graph_id (Optional[str]): The target graph ID. Defaults to "default" if not specified.
        ...
        try:
            ...
            # Add to background tasks with graph context
            if MULTI_GRAPH_SUPPORT and graph_id != "default":
                background_tasks.add_task(pipeline_index_file_with_graph, rag, file_path, graph_id)
            else:
    ...
    async def documents(graph_id: Optional[str] = None) -> DocsStatusesResponse:
        """
        ...
        try:
            ...
            for idx, result in enumerate(results):
                ...
                for doc_id, doc_status in result.items():
                    ...
                    # 获取文档的图谱信息（简化实现）
                    doc_graph_id = getattr(doc_status, 'graph_id', 'default')
                    doc_graph_name = getattr(doc_status, 'graph_name', 'Default Graph')
