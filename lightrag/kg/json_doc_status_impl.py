...
class JsonDocStatusStorage(DocStatusStorage):
    ...
    async def initialize(self):
        ...
        new_file_path = os.path.join(
            self.global_config["working_dir"], f"kv_store_{self.namespace}.json"
        )

        # 如果文件路径发生变化，重新加载数据
        if new_file_path != self._file_name:
            logger.info(f"Doc status working directory changed, reloading from {new_file_path}")
            self._file_name = new_file_path
    ...
    def update_working_dir(self, new_working_dir: str):
        """Update working directory and reload data if path changed"""
        new_file_path = os.path.join(new_working_dir, f"kv_store_{self.namespace}.json")

        if new_file_path != self._file_name:
            logger.info(f"Doc status working directory changed from {self._file_name} to {new_file_path}")
            self._file_name = new_file_path
            ...
            if loaded_data:
                ...
                self._data.clear()
                self._data.update(loaded_data)
                logger.info(f"Doc status reloaded {len(loaded_data)} records from {new_file_path}")
            else:
                logger.info(f"No doc status data found at {new_file_path}")
