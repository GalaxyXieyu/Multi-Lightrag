...
const GraphLabels = () => {
  ...
  // Reset graph data when global graph changes
  useEffect(() => {
    if (globalGraphUpdateTime > 0) {
      console.log('Global graph changed, resetting graph data...')
      const graphState = useGraphStore.getState()

      // Reset graph data to trigger fresh load
      graphState.reset()
      graphState.setGraphDataFetchAttempted(false)
      graphState.setLabelsFetchAttempted(false)

      // Clear current query label to force reload
      useSettingsStore.getState().setQueryLabel('*')
    }
  }, [globalGraphUpdateTime])
