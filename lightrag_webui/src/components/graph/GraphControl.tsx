...
const GraphControl = ({ disableHoverEffect }: { disableHoverEffect?: boolean }) => {
  ...
  useEffect(() => {
    ...
    const events: Record<string, any> = {
      ...
      // 右键节点显示菜单
      rightClickNode: (event: NodeEvent) => {
        const originalEvent = event.event.original as MouseEvent
    ...
    if (enableEdgeEvents) {
      ...
      // 右键边显示菜单
      events.rightClickEdge = (event: EdgeEvent) => {
        const originalEvent = event.event.original as MouseEvent
        originalEvent.preventDefault()
        const graph = sigma.getGraph()
        setContextMenu({
          ...
          y: originalEvent.clientY,
          edgeId: event.edge,
          sourceId: graph.source(event.edge),
          targetId: graph.target(event.edge)
        })
      }
    }
  ...
  }, [registerEvents, enableEdgeEvents, setContextMenu])
