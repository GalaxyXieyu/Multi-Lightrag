...
import { RefreshCwIcon, ActivityIcon, ArrowUpIcon, ArrowDownIcon, FilterIcon, DatabaseIcon } from 'lucide-react'
...
export default function DocumentManager() {
  ...
  const filteredAndSortedDocs = useMemo(() => {
    ...
    if (graphFilter !== 'all') {
      filteredDocuments = allDocuments.filter(doc => {
        const docGraphId = (doc as any).graph_id || (doc as any).graph_name || 'Default';
        return docGraphId === graphFilter;
      });
    }
  ...
  }, [docs, sortField, sortDirection, statusFilter, graphFilter, sortDocuments]);
  ...
  const fetchDocuments = useCallback(async () => {
    try {
      ...
      // 获取当前图谱ID
      const currentGraph = useGraphState.getState().currentGraph;
      const graphId = currentGraph?.graph_id;

      const docs = await getDocuments(graphId);
  ...
  // Monitor global graph state changes
  const currentGlobalGraph = useGraphState.use.currentGraph()
  const globalGraphUpdateTime = useGraphState.use.lastUpdateTime()

  // Refresh documents when global graph changes
  useEffect(() => {
    if (currentTab === 'documents' && globalGraphUpdateTime > 0) {
      fetchDocuments()
    }
  }, [currentTab, globalGraphUpdateTime, fetchDocuments])
                  ...
                  <Table className="w-full">
                    <TableHeader className="sticky top-0 bg-background z-10 shadow-sm">
                      <TableRow className="border-b bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/75 shadow-[inset_0_-1px_0_rgba(0,0,0,0.1)]">
                        ...
                        <TableHead>{t('documentPanel.documentManager.columns.graph', 'Graph')}</TableHead>
                          ...
                          <TableCell>
                            <div className="flex items-center gap-1">
                              <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200">
                                {(doc as any).graph_name || (doc as any).graph_id || 'Default'}
                              </span>
                            </div>
                          </TableCell>
                          <TableCell>
