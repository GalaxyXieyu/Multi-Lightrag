...
import { checkHealth, LightragStatus, getCurrentGraph, GraphInfo } from '@/api/lightrag'
...
const useGraphStateStoreBase = create<GraphState>()((set, get) => ({
  ...
  setCurrentGraph: (graph: GraphInfo | null) => {
    set({
      currentGraph: graph,
      lastUpdateTime: Date.now()
    })
  ...
  refreshCurrentGraph: async () => {
    await get().loadCurrentGraph()
  }
}))

export const useGraphState = createSelectors(useGraphStateStoreBase)
